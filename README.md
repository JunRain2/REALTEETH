...
| 인덱스 | 컬럼 | 목적 |
|---|---|---|
| `idx_tooth_analyses_status_last_sent_at` | `(status, last_sent_at)` | PendingAnalysisScheduler / ProcessingAnalysisScheduler 조회 최적화 |
| `idx_tooth_analyses_created_at` | `(created_at)` | 목록 조회 날짜 범위 필터 |
| `idx_tooth_analyses_status_created_at` | `(status, created_at)` | 상태 + 날짜 복합 필터 조회 최적화 |
